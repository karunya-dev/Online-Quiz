...
import { initSchema, pool, contentDb } from "./db.js";
...
async function generateReport(sessionId: string) {
  const { rows: participantRows } = await pool.query("select * from participants where session_id = $1", [sessionId]);
  const { rows: sessionRows } = await pool.query("select * from sessions where id = $1", [sessionId]);
let questionCount = 0;
try {
  const { rows: questionRows } = await pool.query(
    "SELECT COUNT(*)::int AS count FROM resource_questions WHERE resource_id = $1",
    [sessionRows[0].resource_id]
  );
  questionCount = questionRows[0].count || 0;
} catch (e) {
  // fallback to Mongo content DB
  try {
    const db = await contentDb();
    const resource = await db.collection("activities").findOne({ _id: sessionRows[0].resource_id });
    questionCount = resource?.questions?.length || 0;
  } catch (err) {
    questionCount = 0;
  }
}
...
const maxScore = Math.max(questionCount * 100, 1);
  const average = participantRows.length
    ? Math.round(participantRows.reduce((sum, item) => sum + Number(item.score), 0) / participantRows.length / maxScore * 100)
    : 0;
  const completed = participantRows.filter((item) => Number(item.score) >= maxScore).length;
  const completion = participantRows.length ? Math.round((completed / participantRows.length) * 100) : 0;
  await pool.query(
    `insert into reports (id, session_id, average_score, participant_count, completion_rate)
     values ($1, $2, $3, $4, $5)
     on conflict (session_id) do update set average_score = $3, participant_count = $4, completion_rate = $5`,
    [`RPT-${sessionId}`, sessionId, average, participantRows.length, completion]
  );
}
